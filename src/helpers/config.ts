...
export const showConfigUI = async () => {
  try {
    ...
    const choice = (await p.select({
      ...
      options: [
        ...
        },
        {
          label: 'Gemini Model',
          value: 'GEMINI_MODEL',
          hint: hasOwn(config, 'GEMINI_MODEL') ? config.GEMINI_MODEL : '(not set)',
        },
        {
          label: 'Done',
          value: 'cancel',
    ...
    if (choice === 'OPENAI_KEY') {
      ...
      if (p.isCancel(key)) return;
      await setConfigs([['ANTHROPIC_KEY', key as string]]);
      ...
      if (p.isCancel(key)) return;
      await setConfigs([['GEMINI_KEY', key as string]]);
    ...
    } else if (choice === 'GEMINI_MODEL') {
      const model = await p.text({
        message: 'Enter the Gemini model you want to use',
      });
      if (p.isCancel(model)) return;
...
//     return model as TiktokenModel;
//   },
//   'gemini-1.5-pro-exp-0827'(model?: string) {
//     return model || 'gemini-1.5-pro-exp-0827';
//    },
//   ANTHROPIC_MODEL(model?: string) {
//     if (!model || model.length === 0) {
...
//           hint: hasOwn(config, 'gemini-1.5-pro-exp-0827') ? config['gemini-1.5-pro-exp-0827'] : '(not set)',
//         },
//         {
//           label: 'Done',
//           value: 'cancel',
