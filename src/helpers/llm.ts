...
export const getGemini = async function () {
  const { GEMINI_KEY: geminiKey } = await getConfig();
  if (!geminiKey) {
    throw new KnownError(
...
//   if (useModel === 'gemini-1.5-pro-exp-0827') {
//     if (!GOOGLE_API_KEY) {
//       throw new KnownError(
//         `Missing Google API key. Use \`${commandName} config\` to set it.`
//       );
//     }
//     genai.configure(api_key=GOOGLE_API_KEY);
//     const gemini = genai.GenerativeModel('gemini-1.5-pro-exp-0827');
//     const userMessage = options.messages.find(
//       (message) => message.role === 'user'
//     )?.content as string;
//     process.stdout.write(formatMessage('\n'));
//     const response = await gemini.generate_content(userMessage);
//     process.stderr.write(formatMessage(response.text));
//     process.stdout.write('\n');
//     return response.text;
//   } else if (useAnthropic(useModel)) {
